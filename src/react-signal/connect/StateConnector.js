...
const StateConnector = (NameSpace, selector, events, WrappedComponent) => {
  const Connector = class extends Component {
    namespace = () =>
      Control.withActor(this, NameSpace || this.props.namespace);
    propSelector = typeof selector === "function" ? selector : (props) => props;
    watchedProps = null;

    componentDidMount() {
      this._namespace = NameSpace || this.props.namespace;
      this.displayName = `StateConnector(${
        WrappedComponent.displayName || WrappedComponent.name
      })`;

      if (events && events.length) {
        events.forEach((Event) =>
          this.namespace().listen(Event, () => this.checkState())
        );
      } else {
        this.namespace().listen(StateChanged, () => this.checkState());
        ...
        if (this.watchedProps) {
          this.watchedProps.forEach((prop) => {
            if (typeof this.namespace().state[prop] === "undefined") {
              ...
              console.warn(
                `${WrappedComponent.displayName} was connected with property watcher '${prop}' that is missing from state`,
    ...
    render() {
      ...
      return (
        <WrappedComponent
          {...this.props}
          {...this.propSelector((ns && ns.state) || {})}
        />
      );
    }
  };
  Connector.displayName = `StateConnector(${
    WrappedComponent.displayName || WrappedComponent.name
  })`;

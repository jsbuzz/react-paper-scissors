...
import ReadOnly, { READ_ONLY_PROXY } from "./dependencies/read-only";
...
export class NameSpace extends EventGateway {
  constructor(
    ...
    if (stateDefinition) {
      this.defineState(stateDefinition, readonly, readonlyMethod);
  ...
  defineState(
    stateDefinition,
    readonly = true,
    readonlyMethod = READ_ONLY_PROXY
  ) {
    Control.actor = this;
    let state;

    if (readonly) {
      this._state || (this._state = new ReadOnly({}, readonlyMethod));
    ...
    Object.getOwnPropertyNames(stateDefinition).forEach((property) => {
      ...
      setters.reduce((events, current) => {
        if (current.EventName) return [...events, current];
        ...
        events.forEach((currentEvent) => {
          ...
          this.addEventListener(currentEvent, setter, true);
        });
